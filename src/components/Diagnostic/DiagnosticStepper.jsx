...
export default function DiagnosticStepper() {
  ...
  const calculateDepressionLevel = () => {
    const depressionLevel = Object.keys(expressions).reduce((acc, expr) => acc + expressions[expr], 0);
                ...
                <div className="p-4 border-l border-b border-r border-gray-200 dark:border-gray-700 rounded-b-lg bg-gray-50 dark:bg-gray-800">
                    ...
                    {question.answers.map((answer, i) => (
                      <li key={i}>
                        <div
                          ...
                          className={`w-full text-left p-3 rounded-lg transition-colors duration-300 ${selectedAnswer === i ? 'bg-teal-500 text-white' : 'bg-gray-100 dark:bg-gray-700'}`}
                        >
                          {answer}
                        </div>
                      </li>
                  ...
                  <div className={`mt-6 ${videoActive ? "" : "opacity-60"}`}>
                    <div className="relative w-full h-72 md:h-96 bg-gray-300 rounded-lg flex items-center justify-center dark:bg-gray-700">
                      ...
                      />
                      <canvas key={index} ref={canvasRef} className="absolute top-0 left-0 w-full h-full" />
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={`mt-4 w-full lg:w-auto px-6 py-3 rounded-lg text-lg font-medium tracking-wide transition-all duration-300 ${videoActive ? "bg-red-500 text-white" : "bg-teal-500 text-white"} flex items-center justify-center`}
                  >
                    {videoActive ? (
                      <>
                        <svg
